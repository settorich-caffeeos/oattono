...
1. Supabase Dashboard → **SQL Editor** → วางเนื้อหาไฟล์ `supabase/schema.sql` แล้วกด Run
   (สร้างตาราง `profiles`, `projects`, `documents`, `knowledge_base` + Row Level Security
   + trigger สร้างโปรไฟล์อัตโนมัติตอนสมัคร)
   > ถ้าเคยรัน schema เวอร์ชันก่อนแล้ว ให้รันไฟล์นี้ซ้ำอีกครั้ง — ปลอดภัย (ใช้ `if not exists`)
   > จะเพิ่มตาราง projects/documents/knowledge_base ให้ครบ
